...
- v0.4: Bundle vpto-spec.md with micro ISA chapters and add a standalone PTO TileOp companion doc
...
- **PTO TileOp**: tile-oriented PTO code that serves as a nano-kernel encapsulation of tile operations, primarily expressing computation and data movement in terms of tile buffers, tile shapes, and tile-local layout.
...
- **PTO TileOp+micro Instruction**: a hybrid PTO form that keeps tile-level orchestration while embedding explicit micro-instruction regions where direct vector-pipeline control is required.
...
High-level frameworks / DSLs / library kernels
         ...
         |  (3) PTO TileOp+micro Instruction|
         +----------------+-----------------+
                          |
             +------------+------------+
             |                         |
...
`tileop` is the PTO TileOp surface built on top of PTO micro Instruction. Each `tileop` encapsulates a tile-granular pattern — DMA between GM and on-chip buffers, vector arithmetic over a whole tile, reductions, broadcast / expansion, selection, padding — and internally expands to a sequence of micro-instruction primitives (`pto.vlds`, `pto.vsts`, `pto.vadd`, mask ops, sync flags, …).

The full PTO TileOp reference starts from [Tile and PTO TileOp overview](pto-tileop.md#tile-01-tile-overview). It covers:

- [Tile and PTO TileOp overview](pto-tileop.md#tile-01-tile-overview) — tile concept, on-chip placement, physical shape vs valid region, conventions
- [Types & Attributes](pto-tileop.md#tile-02-types-and-attributes) — `!pto.tile_buf`, `!pto.tensor_view`, address spaces, layout, pad
- [Pointer & View](pto-tileop.md#tile-03-pointer-and-view) — tensor views, partitions, tile allocation, valid-shape updates
- [DMA Data Movement](pto-tileop.md#tile-04-dma-data-movement) — `pto.tload` / `pto.tstore`
- [Vector Arithmetic](pto-tileop.md#tile-05-vector-arithmetic) — `pto.tadd / tsub / tmul / tdiv / tmax / tmin`, tile-scalar forms, unary math, activations
- [Reductions](pto-tileop.md#tile-06-reduction-ops), [Partial Elementwise](pto-tileop.md#tile-07-partial-elementwise), [Bitwise & Shift](pto-tileop.md#tile-08-bitwise-shift-ops), [Type Conversion](pto-tileop.md#tile-09-type-conversion), [Broadcast & Expansion](pto-tileop.md#tile-10-broadcast-and-expansion-ops), [Selection](pto-tileop.md#tile-11-selection-ops), [Fill & Padding](pto-tileop.md#tile-12-fill-and-padding-ops)

For the boundary between `tileop` and the micro instruction surface (when to drop into `pto.vecscope` and how `pto.tile_buf_addr` bridges the two), see [Tile and PTO TileOp overview §1.10](pto-tileop.md#110-mixing-pto-tileop-and-pto-micro-instruction).
